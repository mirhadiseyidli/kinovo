...
>(function GestureContainer(
  ...
  useAnimatedReaction(
    ...
    [refreshHeight, childScrollRef, childScrollYTrans, curIndexValue, tabsTrans]
  );
  ...
  useAnimatedReaction(
    () => {
      "worklet";
      return {
        headerTrans: headerTrans?.value,
        isCurrentIndex: slideIndex.value === curIndexValue.value,
        isSliding: isSlidingHeader.value,
        scrollRef: childScrollRef[curIndexValue.value],
        scrollY: childScrollYTrans[curIndexValue.value]?.value
      };
    },
    (current, previous) => {
      "worklet";
      if (!current.isSliding || !current.isCurrentIndex || !current.headerTrans || !current.scrollRef) {
        return;
      }
      ...
      _ScrollTo(
        current.scrollRef as never,
        0,
        current.headerTrans || 0,
        false
      );
    },
    [childScrollRef, childScrollYTrans, curIndexValue, slideIndex, isSlidingHeader]
  );
  ...
  const headerTransValue = useDerivedValue(() => {
    ...
    return headerTransY;
  }, [shareAnimatedValue, calcHeight, isIOS, animationHeaderPosition]);
