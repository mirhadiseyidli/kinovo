...
export const useGetNumberOfFriendsNewEvents = () => {
  ...
  const fetchNumberOfFriendsNewEvents = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.EXPO_PUBLIC_SERVER_BASE_URL}/api/managefriends/user/get/number/friends/new/events`, {
      ...
      if (error.response?.status === 401) {
        ...
        const retryResponse = await axios.get(`${process.env.EXPO_PUBLIC_SERVER_BASE_URL}/api/managefriends/user/get/number/friends/new/events`, {
