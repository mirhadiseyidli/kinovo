...
export const useCalendarCacheManager = () => {
  ...
  const updateCacheWithRangeEvents = useCallback(async (startDate: Date, endDate: Date) => {
    ...
    try {
      ...
      // Add existing events that weren't in the range
      ...
      // Update the unified cache
      queryClient.setQueryData(QUERY_KEYS.EVENTS, finalEvents);
